...
const LessonDetail = () => {
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      const res = await studentService.summarizeLesson(lessonId, {
      ...
      {courseContent === null && (
        <aside className="w-full md:w-80 lg:w-96 bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 h-fit sticky top-8 max-h-[80vh] overflow-y-auto flex items-center justify-center">
          <span className="text-gray-500 dark:text-gray-400 text-center">
            Không thể tải nội dung khóa học (courseContent=null). Kiểm tra lại
            API lesson detail phải trả về section.course_id.
          </span>
        </aside>
      )}
      {courseContent && (
        <aside className="w-full md:w-80 lg:w-96 bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 h-fit sticky top-8 max-h-[80vh] overflow-y-auto">
          ...
          {courseContent.last_updated_at && (
            <div className="text-xs text-gray-500 dark:text-gray-400 mb-4">
              ...
              {new Date(courseContent.last_updated_at).toLocaleDateString(
                "vi-VN",
                ...
                <div key={section.id}>
                  ...
                  {section.quizzes && section.quizzes.length > 0 && (
                    <div className="mt-2 mb-1">
                      <div className="text-xs font-medium text-blue-700 dark:text-blue-200 mb-1 pl-2">
                        Bài kiểm tra
                      </div>
                      <ul className="space-y-1 ml-4">
                        {section.quizzes.map((q) => (
                          <li key={q.id} className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() =>
                                navigate(`/student/quizzes/${q.id}/history`)
                              }
