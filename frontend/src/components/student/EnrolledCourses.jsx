                ...
                <div className="md:w-1/4">
                  <div className="aspect-video w-full overflow-hidden bg-gray-200 dark:bg-slate-700">
                    {enrollment.course.thumbnail ? (
                      <img
                        src={enrollment.course.thumbnail.startsWith('http') ? enrollment.course.thumbnail : `http://localhost:8000${enrollment.course.thumbnail}`}
                  ...
                  <div className="mt-6">
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-sm font-medium text-indigo-600 dark:text-indigo-400">
                        {(() => {
                          const lessonCount = enrollment.course.lesson_count || 0;
                          const quizCount = typeof enrollment.course.quiz_count === 'number' ? enrollment.course.quiz_count : 0;
                          const total = lessonCount + quizCount;
                          // If backend progress is already correct, just show it. Otherwise, fallback to old logic.
                          return total > 0 ? `${Math.round(enrollment.progress)}%` : `${Math.round(enrollment.progress)}%`;
                        })()}
                      </span>
                    </div>
