...
const StudentQuizPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowResult(true);
      toast.success(`Bạn đã làm đúng ${res.data.correct}/${res.data.total} câu. Điểm: ${res.data.score}/10`);
    } catch (err) {
  ...
  if (!quiz) return <div className="p-8 text-center">Đang tải bài kiểm tra...</div>;

  return (
    <div className="max-w-2xl mx-auto p-6 bg-white dark:bg-slate-800 rounded-xl shadow-md mt-8">
      <h1 className="text-2xl font-bold mb-4 text-indigo-700 dark:text-indigo-200">{quiz.title}</h1>
      {showResult ? (
        <div className="text-center">
          ...
          <div className="flex flex-col gap-3 items-center mt-6">
            ...
            <button
              ...
              onClick={() => navigate(courseId ? `/student/courses/${courseId}` : '/student/courses')}
            >
              Về trang khóa học
            </button>
          </div>
